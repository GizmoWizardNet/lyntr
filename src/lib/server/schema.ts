...
export const users = pgTable('users', {
    ...
    // ── Status bubble ────────────────────────────────────────────────
    // Short free-text status shown in a frosted-glass bubble when someone
    // hovers your username. null/'' = no status set.
    status_text: varchar('status_text', { length: 100 }),
    // When the status expires and should stop being shown. null AND
    // status_text set = "forever" (never expires until manually cleared).
    status_expires_at: timestamp('status_expires_at'),

    // ── Timezone pill ────────────────────────────────────────────────
    // 'GMT' | 'UTC' label prefix the user picked, purely cosmetic.
    timezone_label: text('timezone_label'),
    // Signed offset from the label, e.g. '+05:30', '-08:00', '+00:00'.
    // null = no timezone pill shown on the profile.
...
export const lynts = pgTable('lynts', {
    ...
    // Optional cosmetic background (see LYNTSKIN_CATALOG in
    // src/lib/lyntskins.ts). Set once at creation by whoever owns the skin
    // — for clan lynts that's whoever started the relay — and never
    // editable afterwards. null = no skin.
    lyntskin_key: text('lyntskin_key'),
}, (table) => {
    return {
...
export const clanLynts = pgTable('clan_lynts', {
    ...
    gif_url: text('gif_url'),
    gif_preview_url: text('gif_preview_url'),
    // Chosen by the author when starting the relay — see lynts.lyntskin_key.
    // Carried over verbatim to the resulting lynt on completion; nobody
    // downstream in the relay can change it.
    lyntskin_key: text('lyntskin_key'),
...
export const lcTransactions = pgTable('lc_transactions', {
    ...
    reason: text('reason').notNull(), // 'post_created' | 'like_received' | 'reply_received' | 'bookmark_received' | 'repost_received' | 'curator_bonus' | 'streak_bonus' | 'lyntskin_purchase'
    lynt_id: text('lynt_id').references(() => lynts.id),
...
export const userLyntskins = pgTable('user_lyntskins', {
    id: uuid('id').defaultRandom().primaryKey(),
    user_id: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
    // Key from LYNTSKIN_CATALOG in src/lib/lyntskins.ts, e.g. 'mario'.
    skin_key: text('skin_key').notNull(),
    purchased_at: timestamp('purchased_at').defaultNow(),
}, (table) => {
    return {
        userSkinUnique: uniqueIndex('user_lyntskins_user_id_skin_key_idx').on(table.user_id, table.skin_key),
    }
});
