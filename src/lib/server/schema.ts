...
export const desktopAuthTransactions = pgTable(
    'desktop_auth_transactions',
    {
        id: uuid('id').defaultRandom().primaryKey(),

        // SHA-256 of the random state generated by the desktop client.
        state_hash: text('state_hash').notNull().unique(),

        // SHA-256 of the one-time code generated after successful OAuth.
        code_hash: text('code_hash').unique(),

        // Filled in after the OAuth provider identifies the user.
        ...
        // Set when the one-time exchange code is consumed.
        consumed_at: timestamp('consumed_at'),

        created_at: timestamp('created_at').defaultNow().notNull()
    },
    (table) => ({
        stateIdx: index('desktop_auth_transactions_state_hash_idx').on(
            table.state_hash
        ),

        codeIdx: index('desktop_auth_transactions_code_hash_idx').on(
            table.code_hash
        ),

        expiresIdx: index('desktop_auth_transactions_expires_at_idx').on(
            table.expires_at
        )
    })
);
