...
import { sql, desc, and, eq, exists, or, isNull, not, inArray, asc, gte } from 'drizzle-orm';
...
export async function mainFeed(userId: string, limit = 20, excludePosts: string[] = [], minIq?: number | null) {
	...
	if (minIq !== undefined && minIq !== null) {
		whereConditions = and(whereConditions, gte(users.iq, minIq));
	}
	...
	let feed = await db
		...
		.orderBy(
			...
			desc(sql`CASE WHEN ${users.handle} = 'gizmowizard' THEN 1 ELSE 0 END`),
